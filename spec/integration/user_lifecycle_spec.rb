...
RSpec.describe "Retort user lifecycle integration" do
  ...
  def create_post(owner:)
    ...
    category = Fabricate(:category, user: owner, name: "category-#{token}", slug: "category-#{token}")
    topic = Fabricate(:topic, user: owner, category: category, title: "Retort lifecycle title #{token}")
    Fabricate(:post, topic: topic, user: owner, raw: "This is a verification post body for retort lifecycle #{token}.")
  end
  ...
  describe "user merge" do
    it "moves active retorts to the target user and clears serializer cache" do
      ...
      expect(serializer.retorts).to eq([{ post_id: post.id, usernames: [source_user.username], emoji: "heart" }])
      ...
      post_serializer = PostSerializer.new(post.reload, scope: Guardian.new(target_user), root: false)
    ...
    it "moves soft-deleted retorts to the target user without failing the merge" do
      source_user = create_user("source")
      target_user = create_user("target")
      post = create_post(owner: target_user)
      ...
      expect(User.exists?(source_user.id)).to eq(false)
      moved_retort = Retort.with_deleted.find_by(post_id: post.id, user_id: target_user.id, emoji: "heart")
      expect(moved_retort).to be_present
      expect(moved_retort.deleted_by_id).to eq(target_user.id)
    end

    it "prefers the active source retort over a soft-deleted target duplicate" do
      source_user = create_user("source")
      target_user = create_user("target")
      post = create_post(owner: target_user)
      ...
      retained_retort = Retort.with_deleted.find_by(post_id: post.id, user_id: target_user.id, emoji: "heart")
      expect(retained_retort.id).to eq(source_retort.id)
      expect(retained_retort).not_to be_trashed
    end

    it "prefers the active target retort over a soft-deleted source duplicate" do
      source_user = create_user("source")
      target_user = create_user("target")
      post = create_post(owner: target_user)
    ...
    it "keeps the target active retort when both users have active duplicates" do
      source_user = create_user("source")
      target_user = create_user("target")
      post = create_post(owner: target_user)
      ...
      retained_retort = Retort.with_deleted.find_by(post_id: post.id, user_id: target_user.id, emoji: "heart")
      expect(retained_retort.id).to eq(target_retort.id)
      expect(Retort.with_deleted.where(post_id: post.id, user_id: target_user.id, emoji: "heart").count).to eq(1)
    end

    it "keeps the target soft-deleted retort when both users have soft-deleted duplicates" do
      source_user = create_user("source")
      target_user = create_user("target")
      post = create_post(owner: target_user)
    ...
    it "moves deleted_by_id references from the source user to the target user" do
      source_user = create_user("source")
      target_user = create_user("target")
  ...
  describe "user deletion" do
    it "deletes active retorts owned by the user" do
    ...
    it "deletes soft-deleted retorts owned by the user before the user row is removed" do
      user = create_user("delete")
      post_owner = create_user("owner")
      post = create_post(owner: post_owner)
    ...
    it "deletes active and soft-deleted owned retorts in the same destroy operation" do
      user = create_user("delete")
      post_owner = create_user("owner")
    ...
    it "reassigns deleted_by_id references on other users' retorts to system during user deletion" do
      user = create_user("delete")
      other_user = create_user("other")
